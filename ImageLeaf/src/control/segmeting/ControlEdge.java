...
import view.segmeting.ViewEdge;
...
public class ControlEdge {
    ...
    private ViewEdge view = null;
    private BufferedImage image = null;
    private BufferedImage filterImage = null;

    public ControlEdge(BufferedImage image, viewPrincipal parentFrame) {
        this.image = image;
    ...
    private void initComponents() {
        ...
        view.getRbSobel().setSelected(true);
            ...
            public void actionPerformed(ActionEvent e) {
                SwingWorker work = new SwingWorker() {
                    ...
                    protected Object doInBackground() throws Exception {
                        ...
                        int[][] borderDetect = null;
                        if (view.getRbSobel().isSelected()) {
                            borderDetect = Filtro.bordaSobel(image);
                        } else if (view.getRbPrewwit().isSelected()) {
                            borderDetect = Filtro.bordaPrewitt(image);
                        } else if (view.getRbRoberts().isSelected()) {
                            borderDetect = Filtro.bordaRoberts(image);
                        }
                        filterImage = image;
            ...
            public void actionPerformed(ActionEvent e) {
                SwingWorker work = new SwingWorker() {
                    ...
                    protected Object doInBackground() throws Exception {
                        ...
                        frame.setImageBorder(Limiar.limiarizacao(borderDetect, valueLimiar));
                        parentFrame.repaint();
                        stopProgressBar();
                        view.dispose();
                        return null;
        ...
        //selecionar somente um tipo de masca
        view.getRbSobel().addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                if (view.getRbSobel().isSelected()) {
                    view.getRbPrewwit().setSelected(false);
                    view.getRbRoberts().setSelected(false);
                }
            }
        });

        view.getRbPrewwit().addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                if (view.getRbPrewwit().isSelected()) {
                    view.getRbSobel().setSelected(false);
                    view.getRbRoberts().setSelected(false);
                }
            }
        });

        view.getRbRoberts().addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                if (view.getRbRoberts().isSelected()) {
                    view.getRbPrewwit().setSelected(false);
                    view.getRbSobel().setSelected(false);
                }
            }
